...
def generate_snippets(request, youtube_id):
    ...
    try:
        ...
        # Try to get the transcript in German (prefer manual over auto-generated)
        target_transcripts = []
        if video.available_subtitle_languages:
            try:
                ...
                if not target_transcripts:
                    print("No German transcripts found")
                    messages.error(request, "No German subtitles available for this video.")
                    return redirect('cms:german_with_videos:video_details', youtube_id=youtube_id)
